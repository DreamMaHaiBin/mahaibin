      ...
      <Fragment>
        <Form onSubmit={this.handleSubmit} className="login-form" labelCol={{ span: 5 }} wrapperCol={{ span: 19 }} id="a">
          ...
          <div className="click_show_one">
            <span className="addKuang">添加熔剂数据</span>
            <Form.Item label="名称">
          ...
          this.state.selectedRows.map((item) => {
            return (
              <Form onSubmit={this.handleSubmit} key={item.id}>
                ...
                <div className="click_show_one">
                  <span className="addKuang">修改熔剂数据</span>
                  <Form.Item label="名称">
              {getFieldDecorator('name',{
...
//主体得数据库2的部分
 class Solvent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  UNSAFE_componentWillMount () {
    ...
    axios.get("/api/download/?purpose=7").then((res)=>{
      ////console.log(res);
    this.setState({
      down:res.data.url
    })
    ...
    if (this.state.model === "add") {//新增提交
      ...
      this.refs.editform.validateFields((err, values) => {
        if (err) {
          ...
          axios.put(`/api/sjrjore/${this.state.handleDeleteId[0].id}/`, {
            name: values.name,
            incomingDate: values.incomingDate,
            price: values.price,
            TFe: values.TFe,
    ...
    if(this.state.handleDeleteId.length>0){
      ...
      axios({
       ...
       // //console.log(res)
        ...
        downloadElement.download = '熔剂基础数据.xls'; //下载后文件名
        downloadElement.SheetName="熔剂基础数据"
  ...
  render() {
    ...
    const uploadProps = {
      name: 'myfile',
      action: '/api/orenew/export/?purpose=1',
      headers:{
        Authorization:sessionStorage.getItem("token")
      },
      data: {
    ...
    return (
      <div className="kuang">
            ...
            pagination={
              {
                ...
                onChange: (page, pageSize) => {
                  axios.get(`/api/sjrjore/?page=${page}`,{
                    headers:{
                      Authorization:sessionStorage.getItem("token")
                    }
                  }).then((res) => {
        ...
        <Modal
          title={this.state.model === "add" ? "添加熔剂数据库" : "修改熔剂数据库"}
...
export default withRouter(connect(state => state)(Solvent));