...
const { Option } = Select;
//到导入烧结矿粉
class DaoruGufei extends Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
      ...
      //  console.log(this.props.DataNumber)
    ...
    componentWillMount() {
        let num=this.props.DataInfo===2  || this.props.DataNumber===2 || this.props.DataNumber===1? 7 : 24
        let list=[]
        for (let p = 1; p < num; p++) {
            list.push(p)
        }
        ...
        axios.get(this.props.DataNumber===1 ? "/api/jtname" : this.props.DataInfo===2  || this.props.DataNumber===2? "/api/mfname" :`/api/sjflname/`, {
            headers: {
        ...
        const searchVal = [];
            ...
            for (let i = 0; i < this.state.dataTime.length; i++) {
                if (this.state.dataTime[i] === `${value}`) {
                    ...
                    axios.get(this.props.DataNumber===1 ? `/api/ore-j/?name=${searchVal}` :this.props.DataInfo===2  || this.props.DataNumber===2? `/api/ore-m/?name=${searchVal}` :`/api/sjflore/?name=${searchVal}`, {
                        headers: {
                            Authorization: sessionStorage.getItem("token")
                        }
                    })
    ...
    xuhao(value) {
        this.props.guFeiList.forEach((item, index) => {
            if ((index + 1) === value) {
                //////console.log(value)
                this.setState({
                    xuList: index + 1
...
const WrappedDaoruGufei= Form.create({ name: 'DaoruGufei' })(DaoruGufei);
export default WrappedDaoruGufei