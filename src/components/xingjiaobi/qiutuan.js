...
class EditerComponent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dataListClick: props.ArrayListData,//选中哪一行的信息
...
export default class Abc extends Component {
  ...
  UNSAFE_componentWillMount() {
    axios.get("/api/ore/incoming-date/?purpose=2", {
      headers: {
    ...
    // axios.get("/api/evaluation/recent/", {
    //   headers: {
    //     Authorization: sessionStorage.getItem("token")
    //   }
    // }).then((res) => {
    //   //////console.log(res.data)
    //   this.setState({
    //     prtList: res.data.para.prt,
    //     tlList: res.data.para.tl,
    //     setList: res.data.para
    //   })
    // })
    // axios.get("/api/evaluation/rj/recent/", {
    //   headers: {
    //     Authorization: sessionStorage.getItem("token")
    //   }
    // }).then((res) => {
  ...
  handlersearchinputChange(value) {
    ...
    for (var i = 0; i < this.state.dataTime.length; i++) {
      if (this.state.dataTime[i] === `${value}`) {
        ...
        axios.get(`/api/ore/?ordering=-createTime&incomingDate=${searchVal}&purpose=2`, {
          headers: {
            Authorization: sessionStorage.getItem("token")
          }
        })
  ...
  listNameData(e) {
    ...
    this.state.dataList.forEach((item, index) => {
      if (item.name + ":" + item.incomingDate.slice(0, 8) === e.target.innerHTML) {
        ArrayList.push(item);
        this.setState({
          ArrayListData: ArrayList
        })
  ...
  oneBackList() {
    if (this.state.ArrayListData.length > 0) {
      var dataName = this.state.ArrayListData.slice(0);
      this.setState({
        ArrayListData: [],
      })
    } else {
  ...
  handleOk = (e) => {
    ...
    if (this.state.model === "setAdd") {
      this.refs.EditerComponent.validateFields((err, values) => {
        //////console.log(this.state.ArrayListData)
        this.state.ArrayListData.forEach((item) => {
  ...
  aaaa = (v) => {
    ...
    //////console.log(this.state.zhrjList)
  }
  //排序结构提交
  submitInfo() {
    this.setState({
  ...
  ExportToExcel() {
    ...
    option.fileName = '球团用矿粉导出表'  //导出的Excel文件名
    option.datas = [
      {
        sheetData: dataTable,
        sheetName: '性价比排序结果',
        ...
        <div className="xingjiabi">
          ...
          <table className="tablexinxi">
            <tbody>
              ...
              {
                this.state.ArrayListData.map((item, index) => {
