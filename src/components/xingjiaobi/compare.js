...
import ComputedInfo from "./compareModel/computedInfo.js"
...
const WrappedEditerComponent = Form.create({ name: 'EditerComponent' })(EditerComponent);
//性价比测算排序
class Compare extends Component {
  ...
  UNSAFE_componentWillMount() {
    axios.get("/api/ore/incoming-date/?purpose=1", {
      headers: {
  ...
  handlersearchinputChange(value) {
    ...
    for (var i = 0; i < this.state.dataTime.length; i++) {
      if (this.state.dataTime[i] === `${value}`) {
        ...
        axios.get(`/api/ore/?ordering=-createTime&incomingDate=${searchVal}&purpose=1`, {
          headers: {
            Authorization: sessionStorage.getItem("token")
          }
        })
    ...
    axios.post("/api/evaluation/", {
      ...
      if (res.data.detail) {
        ...
        res.data.result.forEach((item) => {
          ...
          item.ore.priceOre = Number(item.ore.priceOre).toFixed(2);
  ...
  ExportToExcel() {
      ...
      for (let i = 0; i < data.length; i++) {
        if (data) {
          let obj = {
            ...
            '到厂价': Number(data[i].ore.priceOre).toFixed(2),
    ...
    option.fileName = '烧结用矿粉导出表'  //导出的Excel文件名
    option.datas = [
      {
        sheetData: dataTable,
        sheetName: '性价比排序结果',
    ...
    return (
      <div>
        <span className="right">
          ...
          <button className="gongneng" onClick={this.ExportToExcel.bind(this)}>
            <img src={require("../../img/btn-export.png")} alt="" /> 生成报表</button>
        </span>
        <div className="xingjiabi">
          ...
          <div className="anniu">
            <div className="anniu-center">
              <Button onClick={this.removeList.bind(this)}>
                <img src={require("../../img/arrow04.png")} alt="单选" />
              </Button>
                ...
                this.state.ArrayList.map((item, index) => {
                  return (
                    <tr key={index} className="shujutd">
                      ...
                      <td>{Number(item.priceOre).toFixed(2)}</td>
                    </tr>
                  )
                })
              }
        ...
        >
          {
            //参数设置   修改设置   膨润土  综合熔剂
            this.state.model === "set" ? <WrappedAddComponentFrom ref="addcomponent" setList={this.state.setList} /> : (this.state.model === "setAdd" ? <WrappedEditerComponent ref="EditerComponent" set={this.state.ArrayList} /> :
...
export default withRouter(connect(state => state)(Compare));