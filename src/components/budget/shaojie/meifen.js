...
for (let i = 0; i < 10; i++) {
  ...
  export default  class MeiFen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            guFeiList:listTwo,
    ...
    conputedGuFeiResponse() {
        ...
        axios.post("/api/estimate-mean/", {
            ...
            headers: {
                ...
                this.props.getMeiFen(this.state.ListDataFufeiInfo)
        })
            .catch(err => {
                this.setState({
                    flag: false,
    ...
    render() {
        return (
            <div>
                ...
                <Button className="computed" onClick={this.conputedGuFeiResponse.bind(this)} disabled={this.state.flag}>计算煤粉平均价格和成分</Button>
                <Button className="import" onClick={this.import.bind(this)} disabled={this.state.flag}>导入煤粉</Button>
                <Modal
                    title="导入煤粉"
