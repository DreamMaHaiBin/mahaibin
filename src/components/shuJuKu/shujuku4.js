...
import { Table, Modal, Input, message,  Upload, Select, Form, DatePicker } from "antd";
      ...
      <Fragment>
        <Form onSubmit={this.handleSubmit} >
          <div className="click_show_one">
            <span className="addKuang">添加护炉料矿粉</span>
            <Form.Item label="名称">
            ...
            <Form.Item label="护炉料类型">
              {getFieldDecorator('burdenType')(
                <Select style={{ width: 90, padding: 0, fontSize: 12 }} onChange={this.selectedINfo.bind(this)}>
                  <Option value="钛粉">钛粉</Option>
                  <Option value="钛球">钛球</Option>
                  <Option value="钛矿">钛矿</Option>
          ...
          this.state.selectedRows.map((item) => {
            return (
              <Form onSubmit={this.handleSubmit} key={item.id}>
                ...
                <div className="click_show_one">
                  <span className="addKuang">修改护炉料矿粉</span>
                  <Form.Item label="名称">
                    {getFieldDecorator('name', {
                      // rules: [{ required: true, message: '!' }],
...
//主体得数据库2的部分
 class ShuJuKu4 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  componentWillMount   () {
    axios.get(`/api/ore/?ordering=-createTime&incomingDate=&purpose=4`,{
      headers:{
        Authorization:sessionStorage.getItem("token")
      }
    }).then((res) => {
      res.data.results.forEach((item, index) => {
        if (item.burdenType === 6) {
          item.burdenType = "钛矿"
        } else if (item.burdenType === 4) {
          item.burdenType = "钛粉"
        } else if (item.burdenType === 5) {
          item.burdenType = "钛球"
    ...
    axios.get("/api/download/?purpose=4").then((res)=>{
      //console.log(res);
    this.setState({
      down:res.data.url
    })
    ...
    if (this.state.model === "add") {//新增提交
      this.refs.addform.validateFields((err, values) => {
        if (err) {
          ...
          axios.post("/api/ore/?incomingDate=&purpose=4", {
            source: {
            ...
            burdenType: values.burdenType === "钛粉" ? 4 : (values.burdenType === "钛球" ? 5 : (values.burdenType === "钛矿" ? 6 : (values.burdenType === "未知类型" ? 7 : ""))),//burdenType
      ...
      this.refs.editform.validateFields((err, values) => {
        if (err) {
          ...
          axios.put(`/api/ore/${this.state.handleDeleteId[0].id}/?incomingDate=&purpose=4/`, {
            source: {
              tFe: values.source.tFe,
              siO2: values.source.siO2,
              al2O3: values.source.al2O3,
    ...
    if(this.state.handleDeleteId.length>0){
      ...
      axios({
       ...
       // //console.log(res)
        ...
        downloadElement.download = '护炉料基础数据.xls'; //下载后文件名
        downloadElement.SheetName="护炉料基础数据"
  ...
  render() {
    ...
    const { shujuku4Data } = this.props
    //console.log(shujuku4Data);
    return (
      <div className="kuang">
        <div>
  ...
  <div className="gongneng_big">
            ...
            pagination={
              {
                ...
                onChange: (page, pageSize) => {
                  axios.get(`/api/ore/?incomingDate=&ordering=-createTime&page=${page}&purpose=4`,{
                    headers:{
                      Authorization:sessionStorage.getItem("token")
                    }
                  }).then((res) => {
        ...
        <Modal
          title={this.state.model === "add" ? "添加护炉料数据库" : "修改护炉料数据库"}
...
export default withRouter(connect(state => state)(ShuJuKu4));