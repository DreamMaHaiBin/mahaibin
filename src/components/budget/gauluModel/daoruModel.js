...
componentWillMount() { 
    axios.get((this.props.GaoList.length===15 ? `/api/importfunctions/?purpose=3` : this.props.GaoList[0].sign ?  "/api/mfname/" :"/api/jtname/"), {
        headers: {
    ...
    //console.log(this.props.GaoList.length)
    const list = []
    for (let i = 1; i < (this.props.GaoList.length===15 ? 7 : 12); i++) {
        list.push(i)
    }
    this.setState({
        list: list
...
handlersearchinputChange(value) {
    ...
    for (var i = 0; i < this.state.dataTime.length; i++) {
        if (this.state.dataTime[i] === `${value}`) {
            ...
            axios.get((this.props.GaoList.length===15 ? `/api/ore/?ordering=-createTime&name=${searchVal}&purpose=3` :  this.props.GaoList[0].sign ? `/api/ore-m/?name=${searchVal}` :`/api/ore-j/?name=${searchVal}`), {
                headers: {
                    Authorization: sessionStorage.getItem("token")
                }
            })
...
xuhao(value) {
    //console.log(this.props.GaoList);
    
    this.props.GaoList.forEach((item, index) => {
        if ((index + 1) === value) {
            //////console.log(value)
