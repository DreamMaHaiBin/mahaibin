...
class EditerComponent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dataListClick: props.ArrayListXz,//选中哪一行的信息
...
//性价比测算排序
export default class Bcd extends Component {
  constructor(props) {
    super(props);
    //////console.log(props)
    this.state = {
  ...
  UNSAFE_componentWillMount() {
    axios.get("/api/ore/incoming-date/?purpose=3", {
      headers: {
  ...
  handlersearchinputChange(value) {
    ...
    for (var i = 0; i < this.state.dataTime.length; i++) {
      if (this.state.dataTime[i] === `${value}`) {
        ...
        axios.get(`/api/ore/?ordering=-createTime&incomingDate=${searchVal}&purpose=3`, {
          headers: {
            Authorization: sessionStorage.getItem("token")
          }
        })
  ...
  oneList() {
    ...
    if (this.state.ArrayListXz.length > 0) {
      var dataList = this.state.ArrayListXz.slice(0);
      this.setState({ ArrayListXz: [] })
  ...
  oneBackList() {
    if (this.state.ArrayListXz.length > 0) {
      var dataName = this.state.ArrayListXz.slice(0);
      this.setState({
        ArrayListXz: [],
      })
    } else {
  ...
  handleOk = (e) => {
    ...
    if (this.state.model === "setAdd") {
      this.refs.EditerComponent.validateFields((err, values) => {
        //////console.log(this.state.ArrayListXz)
        this.state.ArrayListXz.forEach((item) => {
      ...
      para: {
        ...
        rj:
          this.state.zhrjList.map((items, index) => {
            ...
            if (items.radioMonth === "" || items.radioMonth === "0.00") {
              items.radioMonth = 0;
            }
            if (items.priceMonth === "" || items.priceMonth === "0.00") {
              items.priceMonth = 0;
            }
            if (items.siO2 === "" || items.siO2 === "0.00") {
              items.siO2 = 0;
            }
            if (items.caO === "" || items.caO === "0.00") {
              items.caO = 0;
            }
            return (
              {
                line: items.line,
                name: items.name,
                radioMonth: items.radioMonth,
                priceMonth: items.priceMonth,
                siO2: items.siO2,
                caO: items.caO,
              }
            )
  ...
  ExportToExcel() {
    ...
    option.fileName = '入炉料导出表'  //导出的Excel文件名
    option.datas = [
      {
        sheetData: dataTable,
        sheetName: '性价比排序结果',
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="xingjiabi">
          ...
          <table className="tablexinxi">
            <tbody>
              ...
              {
                this.state.ArrayListXz.map((item, index) => {
        ...
        >
          {      //参数设置   修改设置   膨润土  综合熔剂
            this.state.model === "set" ? <WrappedAddComponentFrom ref="addcomponent" setList={this.state.setList} /> : (this.state.model === "setAdd" ? <WrappedEditerComponent ref="EditerComponent" set={this.state.ArrayListXz} /> :
              (this.state.model === "Bentonite" ? <WrappedBentonite ref="Bentonite" prtList={this.state.prtList} /> : (this.state.model === "infoMationZong" ? <ComputedInfo ref="computedInfo" zhrjList={this.state.zhrjList} getData={this.aaaa.bind(this)} setList={this.state.setList} /> :
