...
import "./shujuku1.css"
import { Table, Modal, Input, message, Upload, DatePicker, Form } from "antd";
      ...
      <Fragment>
        <Form onSubmit={this.handleSubmit} className="login-form" labelCol={{ span: 5 }} wrapperCol={{ span: 19 }} id="a">
          ...
          <div className="click_show_one">
            <span className="addKuang">添加煤粉数据</span>
            <Form.Item label="名称">
          ...
          this.state.selectedRows.map((item) => {
            return (
              <Form onSubmit={this.handleSubmit} key={item.id} >
                ...
                <div className="click_show_one">
                  <span className="addKuang">修改煤粉数据</span>
                  <Form.Item label="名称">
                    {getFieldDecorator('name', {
...
//主体得数据库2的部分
 class ShuJuKu5 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  UNSAFE_componentWillMount () {
    axios.get(`/api/ore-m/?ordering=-createTime`,{
      headers:{
    ...
    axios.get("/api/download/?purpose=5").then((res)=>{
      //console.log(res);
    this.setState({
      down:res.data.url
    })
    ...
    if (this.state.model === "add") {//新增提交
      ...
      this.refs.editform.validateFields((err, values) => {
        if (err) {
          ...
          axios.put(`/api/ore-m/${this.state.handleDeleteId[0].id}/`, {
            name: values.name,
      ...
      if(this.state.handleDeleteId.length>0){
        ...
        axios({
         ...
         // //console.log(res)
          ...
          downloadElement.download = '煤粉基础数据.xls'; //下载后文件名
          downloadElement.SheetName="煤粉基础数据"
  ...
  render() {
    ...
    const { shujuku5Data } = this.props
   // console.log(shujuku5Data);
    return (
      <div className="kuang">
        <div>
            ...
            pagination={
              {
                ...
                onChange: (page, pageSize) => {
                  axios.get(`/api/ore/?incomingDate=&ordering=-createTime&page=${page}&purpose=5`,{
                    headers:{
                      Authorization:sessionStorage.getItem("token")
                    }
                  }).then((res) => {
        ...
        <Modal
          title={this.state.model === "add" ? "添加煤粉数据" : "修改煤粉数据"}
...
export default withRouter(connect(state => state)(ShuJuKu5));